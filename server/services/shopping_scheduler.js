...
async function processSchedules() {
  ...
  globalDb.all('SELECT id FROM households', [], async (err, households) => {
    ...
    for (const hh of households) {
      ...
      db.all(
        'SELECT * FROM shopping_schedules WHERE is_active = 1 AND (next_run_date IS NULL OR next_run_date <= ?)',
        [today],
        (err, schedules) => {
          ...
          schedules.forEach(async (schedule) => {
            ...
            db.serialize(() => {
              ...
              db.run('UPDATE shopping_schedules SET next_run_date = ? WHERE id = ?', [
                nextDate,
                schedule.id,
              ]);
            });
