...
describe('⚡ SQLite Concurrency Stress', () => {
    ...
    beforeAll(async () => {
        await request(app).post('/api/auth/register').send({ householdName: 'Stress Test', email: ADMIN_EMAIL, password: PASSWORD, is_test: 1 });
    ...
    test('🧬 WAL Mode Concurrency (20 Parallel Writes)', async () => {
        ...
        console.log(`Concurrency Results: ${results.length - failures.length} Success, ${failures.length} Failed`);
