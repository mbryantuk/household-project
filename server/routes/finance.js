...
const handleUpdateItem = (table) => async (req, res, next) => {
  try {
    ...
    Object.keys(req.body).forEach((k) => {
      if (validColumns.includes(k) && !['id', 'household_id', 'version'].includes(k))
...
/**
 * GET /api/households/:hhId/finance/report
 * Item 222: Trigger/Download monthly report
 */
router.get(
  '/report',
  authenticateToken,
  ...
  async (req, res, next) => {
    try {
      const { generateMonthlyReport } = require('../services/report_generator');
      const now = new Date();
      const filename = await generateMonthlyReport(req.hhId, now.getMonth() + 1, now.getFullYear());

      const filePath = path.join(__dirname, '../data/uploads', filename);
      res.download(filePath, filename, (err) => {
        if (err) logger.error(`[REPORT-DOWNLOAD] Error:`, err);
