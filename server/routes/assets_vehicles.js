...
OBJECT_TABLES.forEach((table) => {
  router.get(`/${table}`, authenticateToken, requireHouseholdRole('viewer'), useTenantDb, async (req, res, next) => {
    try {
      const rows = await dbAll(req.tenantDb, `SELECT * FROM ${table} WHERE household_id = ? AND deleted_at IS NULL`, [req.hhId]);
      response.success(res, decryptData(table, rows || []));
    } catch (err) { next(err); }
  });

  router.get(`/${table}/:id`, authenticateToken, requireHouseholdRole('viewer'), useTenantDb, async (req, res, next) => {
    try {
      const row = await dbGet(req.tenantDb, `SELECT * FROM ${table} WHERE id = ? AND household_id = ? AND deleted_at IS NULL`, [req.params.id, req.hhId]);
      if (!row) throw new NotFoundError(`${table.slice(0,-1)} not found`);
      response.success(res, decryptData(table, row));
    } catch (err) { next(err); }
  });

  router.post(`/${table}`, authenticateToken, requireHouseholdRole('member'), useTenantDb, autoEncrypt(table), async (req, res, next) => {
    try {
  ...
  router.put(`/${table}/:id`, authenticateToken, requireHouseholdRole('member'), useTenantDb, autoEncrypt(table), async (req, res, next) => {
    try {
  ...
  router.delete(`/${table}/:id`, authenticateToken, requireHouseholdRole('member'), useTenantDb, async (req, res, next) => {
    try {
      const result = await dbRun(req.tenantDb, `UPDATE ${table} SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND household_id = ?`, [req.params.id, req.hhId]);
      if (result.changes === 0) throw new NotFoundError(`${table.slice(0,-1)} not found`);
