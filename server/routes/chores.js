...
router.get('/', authenticateToken, (req, res) => {
  ...
  db.all(
    `SELECT c.*, m.name as assignee_name, m.emoji as assignee_emoji 
            FROM chores c 
...
router.put('/:choreId', authenticateToken, (req, res) => {
  ...
  db.run(
    `UPDATE chores SET name = ?, description = ?, assigned_member_id = ?, frequency = ?, value = ?, next_due_date = ?, emoji = ? WHERE id = ? AND household_id = ?`,
    [
      name,
      description,
      assigned_member_id || null,
...
router.post('/:choreId/complete', authenticateToken, (req, res) => {
  ...
  db.get(
    ...
    (err, chore) => {
      ...
      // 2. Log completion
      db.run(
        `INSERT INTO chore_completions (household_id, chore_id, member_id, completed_at, value_earned) VALUES (?, ?, ?, ?, ?)`,
        ...
        (err) => {
          ...
          if (chore.frequency !== 'one_off' && chore.next_due_date) {
            ...
            switch (chore.frequency) {
              case 'daily':
                newDate = addDays(currentDue, 1);
                break;
              case 'weekly':
                newDate = addWeeks(currentDue, 1);
                break;
              case 'monthly':
                newDate = addMonths(currentDue, 1);
                break;
              default:
                newDate = null;
            }
            if (newDate) {
              ...
              db.run(
                `UPDATE chores SET next_due_date = ? WHERE id = ?`,
                [nextDate, chore.id],
                sendResponse
              );
            } else {
