...
router.post('/households', authenticateToken, validate(HouseholdInputSchema), async (req, res) => {
  ...
  try {
    const hhResult = await dbRun(globalDb, `INSERT INTO households (name, is_test) VALUES (?, ?)`, [
      name,
      finalIsTest,
    ]);
...
router.put(
  ...
  async (req, res) => {
    ...
    if (auto_backup !== undefined) {
      ...
      values.push(auto_backup ? 1 : 0);
    }
    if (backup_retention !== undefined) {
      fields.push('backup_retention = ?');
      values.push(parseInt(backup_retention));
    }
    if (enabled_modules !== undefined) {
      fields.push('enabled_modules = ?');
      values.push(enabled_modules);
    }
    if (metadata_schema !== undefined) {
      fields.push('metadata_schema = ?');
      values.push(metadata_schema);
    }
    if (debug_mode !== undefined) {
      fields.push('debug_mode = ?');
      values.push(debug_mode ? 1 : 0);
    }
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await dbRun(
        globalDb,
        `UPDATE users SET default_household_id = NULL WHERE default_household_id = ?`,
        [householdId]
      );
      await dbRun(
        globalDb,
        `UPDATE users SET last_household_id = NULL WHERE last_household_id = ?`,
        [householdId]
      );
      ...
      try {
        if (fs.existsSync(BACKUP_DIR)) {
          ...
          if (backups.length > 0)
            console.log(`[Cleanup] Deleted ${backups.length} backup files for HH ${householdId}`);
        }
      } catch (e) {
  ...
  try {
    ...
    if (link.role === 'admin') {
      const adminCount = await dbGet(
        globalDb,
        `SELECT COUNT(*) as count FROM user_households WHERE household_id = ? AND role = 'admin'`,
        [householdId]
      );
      if (adminCount.count <= 1) {
        return res.status(400).json({
          error:
            'Cannot leave household as the sole administrator. Delete the household or assign another admin first.',
        });
      }
    }

    // 3. Delete the link
    await dbRun(globalDb, `DELETE FROM user_households WHERE user_id = ? AND household_id = ?`, [
      userId,
      householdId,
    ]);
    ...
    await dbRun(
      globalDb,
      `UPDATE users SET default_household_id = NULL WHERE id = ? AND default_household_id = ?`,
      [userId, householdId]
    );
    await dbRun(
      globalDb,
      `UPDATE users SET last_household_id = NULL WHERE id = ? AND last_household_id = ?`,
      [userId, householdId]
    );
...
router.post(
  ...
  async (req, res) => {
    ...
    const { email, role, firstName, lastName, first_name, last_name, avatar, password, is_test } =
      req.body;
      ...
      if (user) {
        ...
        generatedPassword =
          ...
          Array(12)
            ...
            .map((x) => x[Math.floor(crypto.randomInt(0, x.length))])
            .join('');
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (role) {
        await dbRun(
          globalDb,
          `UPDATE user_households SET role = ? WHERE user_id = ? AND household_id = ?`,
      ...
      if (fName !== undefined) {
        uFields.push('first_name = ?');
        uValues.push(fName);
      }
      if (lName !== undefined) {
        uFields.push('last_name = ?');
        uValues.push(lName);
      }
      if (avatar !== undefined) {
        uFields.push('avatar = ?');
