...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      console.log(
        `[SHOPPING] Fetching list for household ${req.params.id} (all=${all}, week_start=${week_start})`
      );
 ...
 */
router.post(
  '/households/:id/shopping-list/copy-previous',
  authenticateToken,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newItem = await dbGet(req.tenantDb, 'SELECT * FROM shopping_items WHERE id = ?', [
        result.id,
      ]);
 ...
 */
router.put(
  '/households/:id/shopping-list/:itemId',
  authenticateToken,
  requireHouseholdRole('member'),
  useTenantDb,
  async (req, res) => {
    ...
    try {
      ...
      const updatedItem = await dbGet(req.tenantDb, 'SELECT * FROM shopping_items WHERE id = ?', [
        itemId,
      ]);
 ...
 */
router.delete(
  '/households/:id/shopping-list/clear',
  authenticateToken,
  requireHouseholdRole('member'),
  useTenantDb,
  async (req, res) => {
...
router.delete(
  ...
  async (req, res) => {
    try {
      await dbRun(req.tenantDb, 'DELETE FROM shopping_items WHERE household_id = ? AND id = ?', [
        req.params.id,
        req.params.itemId,
      ]);
