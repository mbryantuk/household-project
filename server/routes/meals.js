...
router.get('/', authenticateToken, requireHouseholdRole('viewer'), useTenantDb, async (req, res, next) => {
  try {
    const rows = await dbAll(req.tenantDb, 'SELECT * FROM meals WHERE household_id = ? AND deleted_at IS NULL', [req.hhId]);
 ...
 */
router.get('/plan', authenticateToken, requireHouseholdRole('viewer'), useTenantDb, async (req, res, next) => {
  try {
    const rows = await dbAll(req.tenantDb, 'SELECT * FROM meal_plans WHERE household_id = ? AND deleted_at IS NULL', [req.hhId]);
 ...
 */
router.post('/plan', authenticateToken, requireHouseholdRole('member'), useTenantDb, async (req, res, next) => {
  try {
 ...
 */
router.delete('/plan/:itemId', authenticateToken, requireHouseholdRole('member'), useTenantDb, async (req, res, next) => {
  try {
 ...
 */
router.delete('/:itemId', authenticateToken, requireHouseholdRole('member'), useTenantDb, async (req, res, next) => {
  try {
    const result = await dbRun(
      req.tenantDb,
...
router.all("*", (req, res) => res.status(404).json({error: "meals router catch-all", method: req.method, path: req.path}));
