...
router.post('/', authenticateToken, requireHouseholdRole('member'), useTenantDb, (req, res) => {
    ...
    if (is_default) {
        req.tenantDb.run(`UPDATE finance_profiles SET is_default = 0 WHERE household_id = ?`, [req.hhId]);
    }
...
router.put('/:profileId', authenticateToken, requireHouseholdRole('member'), useTenantDb, (req, res) => {
    const { name, emoji, is_default } = req.body;
...
router.delete('/:profileId', authenticateToken, requireHouseholdRole('admin'), useTenantDb, (req, res) => {
