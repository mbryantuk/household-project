...
router.get('/', authenticateToken, requireHouseholdRole('viewer'), useTenantDb, async (req, res) => {
    try {
        const today = new Date();
        ...
        chores.forEach(c => {
            ...
            const item = { type: 'chore', id: c.id, title: c.name, date: c.next_due_date, emoji: c.emoji || '🧹' };
        ...
        events.forEach(e => {
            ...
            const item = { type: 'event', id: e.id, title: e.title, date: e.date, emoji: e.emoji || '📅' };
            
            if (isToday) notifications.info.push({ ...item, message: 'Happening Today' });
            else notifications.upcoming.push({ ...item, message: new Date(e.date).toLocaleDateString() });
        });
        ...
        vehicles.forEach(v => {
            if (v.mot_due) {
                ...
                if (mot <= nextMonth) {
                    ...
                    const item = { 
                        type: 'vehicle', id: v.id, title: `${v.make} ${v.model} MOT`, 
                        date: v.mot_due, emoji: '🔧', 
            ...
            if (v.tax_due) {
                ...
                if (tax <= nextMonth) {
                    ...
                    const item = { 
                        type: 'vehicle', id: v.id, title: `${v.make} ${v.model} Tax`, 
                        date: v.tax_due, emoji: '🧾',
                        message: isUrgent ? 'Expiring Soon!' : 'Due this month'
                    };
                    if (isUrgent) notifications.urgent.push(item);
                    else notifications.upcoming.push(item);
        ...
        const costs = await new Promise((resolve, reject) => {
            req.tenantDb.all(
                `SELECT * FROM recurring_costs WHERE is_active = 1`, 
                [], 
                (err, rows) => err ? reject(err) : resolve(rows || [])
            );
        });
        ...
        costs.forEach(cost => {
            ...
            if (nextDue && nextDue >= today && nextDue <= nextWeek) {
                ...
                const item = { 
                    type: 'bill', id: cost.id, title: cost.name, 
                    date: nextDue.toISOString().split('T')[0], 
                    amount: cost.amount, emoji: cost.emoji || '💸' 
                };
