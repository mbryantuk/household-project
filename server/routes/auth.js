...
router.post('/lookup', async (req, res) => {
  ...
  try {
    const user = await dbGet(
      globalDb,
      `SELECT first_name, avatar FROM users WHERE email = ? OR username = ? COLLATE NOCASE`,
...
router.post('/mfa/disable', authenticateToken, async (req, res) => {
  ...
  try {
    const user = await dbGet(globalDb, `SELECT password_hash FROM users WHERE id = ?`, [
      req.user.id,
    ]);
    const isValid = bcrypt.compareSync(password, user.password_hash);
    if (!isValid) return res.status(403).json({ error: 'Invalid password' });

    await dbRun(globalDb, `UPDATE users SET mfa_enabled = 0, mfa_secret = NULL WHERE id = ?`, [
      req.user.id,
    ]);
...
router.post('/mfa/login', async (req, res) => {
  ...
  if (!preAuthToken || !code)
    return res.status(400).json({ error: 'Code and pre-auth token required' });
...
router.delete('/sessions', authenticateToken, async (req, res) => {
  try {
    await dbRun(globalDb, `DELETE FROM user_sessions WHERE user_id = ? AND id != ?`, [
      req.user.id,
      req.user.sid,
    ]);
...
router.delete('/sessions/:id', authenticateToken, async (req, res) => {
  try {
    await dbRun(globalDb, `DELETE FROM user_sessions WHERE id = ? AND user_id = ?`, [
      req.params.id,
      req.user.id,
    ]);
...
router.get('/profile', authenticateToken, async (req, res) => {
  try {
    const user = await dbGet(
      globalDb,
      `SELECT id, email, username, first_name, last_name, avatar, system_role, dashboard_layout, sticky_note, theme, mode, custom_theme, default_household_id FROM users WHERE id = ?`,
      [req.user.id]
    );
...
router.put('/profile', authenticateToken, async (req, res) => {
  ...
  if (custom_theme !== undefined) {
    ...
    values.push(custom_theme);
  }
  if (sticky_note !== undefined) {
    fields.push('sticky_note = ?');
    values.push(sticky_note);
  }
  if (default_household_id !== undefined) {
    fields.push('default_household_id = ?');
    values.push(default_household_id);
  }
