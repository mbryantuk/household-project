  ...
  async (req, res) => {
    ...
    try {
      ...
      const [
        ...
        dbAll(`SELECT * FROM recurring_costs WHERE household_id = ? AND is_active = 1`, [
          householdId,
        ]),
      ...
      generateEvents(
        creditCards.map((cc) => ({ ...cc, frequency: 'monthly', day_of_month: cc.payment_day })),
        'bill',
      ...
      generateEvents(
        savings.map((s) => ({ ...s, frequency: 'monthly', day_of_month: s.deposit_day })),
        'saving',
      ...
      generateEvents(
        investments.map((i) => ({ ...i, frequency: 'monthly', day_of_month: i.deposit_day })),
        'saving',
        (i) => `📈 Investment: ${i.name}`,
        (i) => i.emoji || '📈',
      ...
      generateEvents(
        pensions.map((p) => ({ ...p, frequency: 'monthly', day_of_month: p.payment_day })),
        'saving',
