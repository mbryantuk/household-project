...
router.post('/analyze-receipt', authenticateToken, requireHouseholdRole('member'), useTenantDb, upload.single('receipt'), async (req, res) => {
    ...
    try {
        ...
        lines.forEach(line => {
            ...
            const skipTerms = ['TOTAL', 'SUBTOTAL', 'TAX', 'VAT', 'CASH', 'CHANGE', 'DATE', 'THANK', 'STORE', 'ADDRESS'];
            if (skipTerms.some(term => trimmed.toUpperCase().includes(term))) return;

            const match = trimmed.match(priceRegex);
            if (match) {
                const price = parseFloat(match[1]);
                const name = trimmed.replace(match[0], '').trim().replace(/^[^\w]+/, ''); // Clean non-word starts
                if (name.length > 2) {
                    items.push({ name, estimated_cost: price, quantity: '1', category: 'general' });
                }
            } else if (trimmed.length > 5 && !trimmed.includes(':') && !trimmed.match(/^\d+$/)) {
                // Potential item without price
                items.push({ name: trimmed, estimated_cost: 0, quantity: '1', category: 'general' });
            }
        });
