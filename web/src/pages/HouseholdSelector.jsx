...
export default function HouseholdSelector({
  ...
  const handleDeleteHousehold = async (e, hh) => {
    ...
    confirmAction(
      'Delete Household',
      `⚠️ Are you sure you want to DELETE ${hh.name}? This action is permanent and will destroy all data.`,
      async () => {
        try {
  ...
  const handleLeaveHousehold = async (e, hh) => {
    ...
    confirmAction(
      'Leave Household',
      `Are you sure you want to leave ${hh.name}? You will lose access until invited back.`,
      async () => {
        try {
  ...
  const handleExportHousehold = async (e, hh) => {
    ...
    try {
      ...
      const downloadRes = await api.get(`/households/${hh.id}/backups/${filename}`, {
        responseType: 'blob',
      });
    ...
    >
      <Container maxWidth="md">
        <Box sx={{ mb: 6, textAlign: 'center' }}>
          <Typography level="h1" fontWeight="xl" mb={1}>
        ...
        <Grid container spacing={3} justifyContent="center">
            ...
            <Grid key={hh.id} xs={12} sm={6} md={4}>
              ...
              >
                <AspectRatio
                  ratio="1"
                  variant="soft"
                  sx={{ borderRadius: '50%', mb: 2, bgcolor: getEmojiColor(hh.avatar || '🏠') }}
                >
                  <Box
                    sx={{
                      display: 'flex',
                  ...
                  {hh.role !== 'admin' && (
                    <Tooltip title="Leave Household" variant="soft">
                      <IconButton
                        ...
                        color="danger"
                        size="sm"
                        onClick={(e) => handleLeaveHousehold(e, hh)}
                      >
                        <ExitToApp />
                      </IconButton>
                    </Tooltip>
          ...
          <Grid xs={12} sm={6} md={4}>
            ...
            >
              ...
              <Typography level="body-xs" textAlign="center" sx={{ px: 2 }}>
                Register a new property tenant
              </Typography>
            </Card>
          </Grid>
        </Grid>
        ...
        <Box
          sx={{ mt: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}
        >
          <Button variant="plain" color="neutral" startDecorator={<Logout />} onClick={onLogout}>
            Logout
          </Button>
        </Box>
