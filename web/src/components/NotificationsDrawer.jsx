...
import { Drawer, Sheet, Typography, Box, IconButton, List, ListItem, ListItemContent, ListItemDecorator, Chip, Divider, Button, Badge } from '@mui/joy';
import { Close, Notifications, CheckCircle, Warning, Info, CalendarMonth } from '@mui/icons-material';
...
export default function NotificationsDrawer({ open, onClose }) {
    ...
    useEffect(() => {
        if (open && household?.id) {
            api.get(`/households/${household.id}/notifications`)
    ...
    const renderSection = (title, items, color, icon) => {
        ...
        return (
            <Box sx={{ mb: 2 }}>
                <Typography level="title-sm" startDecorator={icon} color={color} sx={{ mb: 1, px: 2, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.75rem' }}>
                    {title}
                </Typography>
                    ...
                    {items.map((n, i) => (
                        <ListItem key={i}>
                            ...
                            <ListItemContent sx={{ cursor: 'pointer' }} onClick={() => handleItemClick(n)}>
                                ...
                                <Typography level="body-xs">{n.message} • {new Date(n.date).toLocaleDateString()}</Typography>
                            </ListItemContent>
                            {n.amount && <Chip size="sm" variant="soft" color={color}>{n.amount}</Chip>}
                        </ListItem>
                    ))}
                </List>
            </Box>
    ...
    const totalCount = (notifications.urgent?.length || 0) + (notifications.upcoming?.length || 0) + (notifications.info?.length || 0);
    ...
    return (
        ...
        >
            <Sheet sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'transparent' }}>
                <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid', borderColor: 'divider' }}>
                    <Typography level="title-lg" startDecorator={<Notifications color="primary" />}>
                        Notifications
                        {totalCount > 0 && <Chip size="sm" color="primary" variant="soft" sx={{ ml: 1 }}>{totalCount}</Chip>}
                    </Typography>
                ...
                <Divider />
                <Box sx={{ p: 2 }}>
                    <Button fullWidth variant="soft" color="neutral" onClick={onClose}>Dismiss All</Button>
                </Box>
            </Sheet>
        </Drawer>
    );
