...
export default function FinanceCard({
    ...
    const formatCurrency = (val) => {
        ...
        let code = currency === '£' ? 'GBP' : (currency === '$' ? 'USD' : (currency === '€' ? 'EUR' : currency));
        try {
    ...
    return (
        <Card variant="outlined" sx={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <Avatar size="lg" sx={{ bgcolor: getEmojiColor(emoji, isDark) }}>{emoji}</Avatar>
                <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                    <Typography level="title-lg" noWrap>{title}</Typography>
                    <Typography level="body-sm" noWrap>{subtitle}</Typography>
                </Box>
                <Box sx={{ textAlign: 'right' }}>
                    <Typography level="h3" color={balanceColor}>{formatCurrency(balance)}</Typography>
                    ...
                    {subValue && (
                        ...
                        <Typography level="body-xs" color="neutral">{subValue}</Typography> :
                        subValue
                    )}
                </Box>
            </Box>
