...
import { Box, Typography, Stack, CircularProgress, Chip, List, ListItem, ListItemContent, ListItemDecorator } from '@mui/joy';
...
export default function IncomeWidget({ api, household }) {
  ...
  if (loading) return <WidgetWrapper title="Income" icon={<AttachMoney />} color="success"><CircularProgress size="sm" /></WidgetWrapper>;
    ...
    <WidgetWrapper title="Income" icon={<AttachMoney />} color="success">
      <Stack spacing={2}>
        <Box>
            ...
            <Typography level="h4" color="success">£{total.toLocaleString()}</Typography>
        </Box>
