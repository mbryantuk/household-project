...
export default function PensionsWidget({ api, household, data, onSaveData }) {
  ...
  const selectedItem = selectedId === 'total' 
    ? null 
    : pensions.find(a => a.id === parseInt(selectedId));

  const totalValue = pensions.reduce((sum, p) => sum + (parseFloat(p.current_value) || 0), 0);
  const totalMonthly = pensions.reduce((sum, p) => sum + (parseFloat(p.monthly_contribution) || 0), 0);
  
  const displayValue = selectedItem ? (parseFloat(selectedItem.current_value) || 0) : totalValue;
  const displayMonthly = selectedItem ? (parseFloat(selectedItem.monthly_contribution) || 0) : totalMonthly;
        ...
        <Box>
            <AppSelect
                ...
                options={[
                    ...
                    ...pensions.map(p => ({ value: String(p.id), label: `${p.emoji || '⏳'} ${p.plan_name}` }))
                ]}
                size="sm"
            />
        </Box>
            ...
            <Stack spacing={1}>
                {pensions.slice(0, 3).map(p => (
                    <Box key={p.id} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Typography level="body-xs">{p.emoji || '⏳'} {p.plan_name}</Typography>
                        <Typography level="body-xs" fontWeight="bold">£{p.current_value?.toLocaleString()}</Typography>
                    </Box>
                ))}
                {pensions.length > 3 && <Typography level="body-xs" color="neutral" textAlign="center">+{pensions.length - 3} more</Typography>}
            </Stack>
        )}

        {pensions.length === 0 && !loading && <Typography level="body-sm" color="neutral" textAlign="center">No pensions found.</Typography>}
