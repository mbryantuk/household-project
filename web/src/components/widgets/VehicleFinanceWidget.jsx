...
export default function VehicleFinanceWidget({ api, household }) {
  ...
  if (loading) return <WidgetWrapper title="Vehicle Finance" icon={<DirectionsCar />} color="danger"><CircularProgress size="sm" /></WidgetWrapper>;
        ...
        <List size="sm" sx={{ '--ListItem-paddingX': 0 }}>
          {data.map(v => (
            <ListItem key={v.id}>
              ...
              <Typography level="title-sm">£{parseFloat(v.remaining_balance || 0).toLocaleString()}</Typography>
            </ListItem>
          ))}
        </List>
      </Stack>
