...
export default function WealthWidget({ api, household }) {
  ...
  const vehicleEquity = useMemo(() => {
    ...
    const financeTotal =
      data.vehicle_finance?.reduce((sum, f) => sum + (f.remaining_balance || 0), 0) || 0;
  ...
  const netWorth = useMemo(() => {
    ...
    const liabilities =
      houseEquity.mortgageTotal +
      vehicleEquity.financeTotal +
      totalLoans +
      totalCreditCards +
      totalOverdrafts;
  ...
  return (
    <WidgetWrapper title="Wealth Tracking" icon={<SavingsIcon />} color="success">
      <Stack spacing={2} sx={{ overflowY: 'auto', pr: 1 }}>
        ...
        >
          <Typography level="body-xs" color="success" textTransform="uppercase" fontWeight="bold">
            Net Worth Estimate
          </Typography>
          <Typography level="h2" color="success" sx={{ my: 0.5 }}>
            ...
            <Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                <Typography
                  level="body-xs"
                  fontWeight="bold"
                  startDecorator={<Home sx={{ fontSize: '1.1rem' }} />}
                >
                  House Equity
                </Typography>
                <Typography level="body-xs" fontWeight="bold">
                  {formatCurrency(houseEquity.equity)}
                </Typography>
              </Box>
              <Typography level="body-xs" color="neutral" sx={{ mb: 0.5 }}>
                {formatCurrency(houseEquity.valuation)} value vs{' '}
                {formatCurrency(houseEquity.mortgageTotal)} debt
              </Typography>
              <LinearProgress
                determinate
                value={Math.min(
                  100,
                  Math.max(0, (1 - houseEquity.mortgageTotal / houseEquity.valuation) * 100)
                )}
                size="sm"
                color="primary"
                sx={{ borderRadius: 'xs' }}
            ...
            <Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                <Typography
                  level="body-xs"
                  fontWeight="bold"
                  startDecorator={<DirectionsCar sx={{ fontSize: '1.1rem' }} />}
                >
                  Vehicle Equity
                </Typography>
                <Typography level="body-xs" fontWeight="bold">
              ...
              <Typography level="body-xs" color="neutral" sx={{ mb: 0.5 }}>
                {formatCurrency(vehicleEquity.value)} value vs{' '}
                {formatCurrency(vehicleEquity.financeTotal)} debt
              </Typography>
              <LinearProgress
                determinate
                value={Math.min(
                  100,
                  Math.max(0, (1 - vehicleEquity.financeTotal / vehicleEquity.value) * 100)
                )}
                size="sm"
                color="warning"
                sx={{ borderRadius: 'xs' }}
              ...
              <Stack spacing={1}>
                ...
                {totalCreditCards > 0 && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography
                      level="body-xs"
                      startDecorator={<CreditCard sx={{ fontSize: '0.9rem' }} />}
                    >
                      Credit Cards
                    </Typography>
                    <Typography level="body-xs" fontWeight="bold" color="danger">
                ...
                {totalOverdrafts > 0 && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography
                      level="body-xs"
                      startDecorator={<AccountBalance sx={{ fontSize: '0.9rem' }} />}
                    >
                      Overdrafts
                    </Typography>
                    <Typography level="body-xs" fontWeight="bold" color="danger">
                      -{formatCurrency(totalOverdrafts)}
                    </Typography>
                  </Box>
                )}
              </Stack>
                    ...
                    {data.savings_pots
                      ...
                      .map((pot) => (
                        <Chip
                          ...
                          variant="soft"
                          color="success"
                          sx={{ fontSize: '0.6rem', height: '18px', '--Chip-paddingInline': '6px' }}
                        >
