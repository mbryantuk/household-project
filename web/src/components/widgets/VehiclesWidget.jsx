...
import { Box, Typography, List, ListItem, ListItemContent, Chip, CircularProgress, Avatar } from '@mui/joy';
        ...
        <List size="sm" sx={{ '--ListItem-paddingY': '8px' }}>
          {vehicles.map(v => (
            <ListItem key={v.id} startDecorator={<Avatar size="sm">{v.emoji || '🚗'}</Avatar>}>
              <ListItemContent>
                <Typography level="body-sm" fontWeight="bold">{v.make} {v.model}</Typography>
                <Box sx={{ display: 'flex', gap: 1 }}>
                    <Chip size="sm" variant={v.mot_due ? 'outlined' : 'soft'} color={v.mot_due ? 'danger' : 'neutral'}>MOT: {v.mot_due || 'N/A'}</Chip>
                    <Chip size="sm" variant={v.tax_due ? 'outlined' : 'soft'} color={v.tax_due ? 'warning' : 'neutral'}>Tax: {v.tax_due || 'N/A'}</Chip>
                </Box>
              </ListItemContent>
            </ListItem>
          ))}
