...
export default function BudgetStatusWidget({ api, household }) {
  ...
  const projection = useMemo(() => {
    ...
    let rawStartDate =
      now.getDate() >= payday
        ? setDate(startOfMonth(now), payday)
        : setDate(startOfMonth(addMonths(now, -1)), payday);

    const getPriorWorkingDay = (date) => {
      let d = new Date(date);
      const isWeekend = (day) => day.getDay() === 0 || day.getDay() === 6;
      const isHoliday = (day) =>
        Array.isArray(bank_holidays) && bank_holidays.includes(format(day, 'yyyy-MM-dd'));
    ...
    const currentCycleRecord = Array.isArray(cycles)
      ? cycles.find((c) => c.cycle_start === cycleKey)
      : null;
    if (!currentCycleRecord)
      return {
        noCycle: true,
        cycleLabel: format(rawStartDate, 'MMMM yyyy'),
        daysRemaining,
        cycleProgress,
      };

    const currentBalance = parseFloat(currentCycleRecord.current_balance) || 0;
    const linkedAccount = Array.isArray(current_accounts)
      ? current_accounts.find((a) => a.id === currentCycleRecord.bank_account_id)
      : null;
    ...
    if (Array.isArray(progress)) {
      progress
        .filter((p) => p.cycle_start === cycleKey)
        .forEach((p) => progressMap.set(p.item_key, p));
    }
      ...
      recurring_costs
        ...
        .forEach((charge) => {
          ...
          datesToAdd.forEach((d) => {
            ...
            const item = {
              key,
              amount: prog?.actual_amount || charge.amount,
              date: d,
              isPaid: prog?.is_paid === 1,
            };
    ...
    incomes.forEach((inc) => {
      ...
      allIncomes.push({
        key,
        amount: prog?.actual_amount || inc.amount,
        date: d,
        isPaid: prog?.is_paid === 1,
      });
    });
    ...
    const pastUnpaidExpenses = allExpenses.filter(
      (e) => isBefore(startOfDay(e.date), now) && !e.isPaid
    );
    ...
    days.forEach((day) => {
      if (isAfter(day, now) || isSameDay(day, now)) {
        const exp = allExpenses
          .filter((e) => isSameDay(e.date, day) && !e.isPaid)
          .reduce((s, e) => s + e.amount, 0);
        const inc = allIncomes
          .filter((i) => isSameDay(i.date, day) && !i.isPaid)
      ...
      <Stack spacing={2}>
        ...
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
            ...
            <Typography level="body-xs" fontWeight="bold">
              {Math.round(projection.billsPct)}%
            </Typography>
          </Box>
          <LinearProgress
            determinate
            value={projection.billsPct}
            thickness={8}
            color="success"
            sx={{ borderRadius: 'xs' }}
          />
        ...
        <Stack spacing={1.5}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Box sx={{ textAlign: 'right' }}>
              ...
              <Typography level="title-sm">{formatCurrency(projection.endOfCycle)}</Typography>
            </Box>
          </Box>
          ...
          {projection.isOverdrawn && (
            <Sheet
              variant="soft"
              color={projection.isLimitRisk ? 'danger' : 'warning'}
              sx={{ p: 1, borderRadius: 'sm', display: 'flex', gap: 1, alignItems: 'center' }}
            >
              <Warning sx={{ fontSize: '1.1rem' }} />
              <Typography level="body-xs" fontWeight="bold">
                {projection.isLimitRisk
                  ? 'CRITICAL: Overdraft Limit Risk'
