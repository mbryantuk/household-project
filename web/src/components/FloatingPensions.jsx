...
import { 
  Close, DragIndicator, OpenInNew, HourglassBottom
} from '@mui/icons-material';
import PensionsContent from './tools/PensionsContent';

export default function FloatingPensions({ onClose, isPopout = false, isDark = false, isDocked = false, onPopout, api, householdId }) {
  ...
  return (
    ...
    >
      <Box onMouseDown={onMouseDown} sx={{ p: 1, bgcolor: 'background.level2', color: 'text.primary', display: 'flex', alignItems: 'center', cursor: isDocked ? 'default' : 'move' }}>
        ...
        <HourglassBottom fontSize="small" sx={{ mr: 1, color: 'primary.plainColor' }} />
        <Typography level="title-sm" sx={{ flexGrow: 1, color: 'inherit' }}>Pensions</Typography>
      ...
      <Box sx={{ flexGrow: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <PensionsContent onClose={onClose} api={api} householdId={householdId} isDark={isDark} />
      </Box>
    </Sheet>
  );
}