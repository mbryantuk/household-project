  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'auto' }}>
      ...
      >
        <TabList
          disableUnderline
          sx={{ p: 0.5, gap: 0.5, borderRadius: 'sm', bgcolor: 'background.level1', mx: 1, mt: 1 }}
        >
          <Tab
            disableIndicator
            ...
            color="success"
            value={0}
            sx={{ borderRadius: 'sm', px: 1, flex: 1, minHeight: 32 }}
          >
            <Savings sx={{ mr: 0.5, fontSize: 18 }} /> Save
          </Tab>
          <Tab
            disableIndicator
            ...
            sx={{ borderRadius: 'sm', px: 1, flex: 1, minHeight: 32 }}
          >
            <AccountBalance sx={{ mr: 0.5, fontSize: 18 }} /> Loan
          </Tab>
          <Tab
            disableIndicator
            ...
            sx={{ borderRadius: 'sm', px: 1, flex: 1, minHeight: 32 }}
          >
            <Home sx={{ mr: 0.5, fontSize: 18 }} /> Mortg
          </Tab>
        </TabList>
        ...
        >
          <FormControl>
            <FormLabel sx={{ mb: 0.5 }}>Starting Amount (£)</FormLabel>
            <Input
              size="sm"
              type="number"
          ...
          <FormControl>
            <FormLabel sx={{ mb: 0.5 }}>Monthly Contribution (£)</FormLabel>
            <Input
              size="sm"
              type="number"
              value={savingsContribution}
              onChange={(e) => setSavingsContribution(e.target.value)}
            />
          </FormControl>
          <Box sx={{ display: 'flex', gap: 1.5 }}>
            <FormControl sx={{ flex: 1 }}>
              <FormLabel sx={{ mb: 0.5 }}>Interest Rate (%)</FormLabel>
              <Input
                size="sm"
                type="number"
                value={savingsRate}
            ...
            <FormControl sx={{ flex: 1 }}>
              <FormLabel sx={{ mb: 0.5 }}>Years</FormLabel>
              <Input
                size="sm"
                type="number"
                value={savingsYears}
          ...
          <Button size="sm" color="success" onClick={calculateSavings}>
            Calculate
          </Button>
          {savingsResult !== null && (
            <Box sx={{ p: 1, bgcolor: 'success.softBg', borderRadius: 'md', textAlign: 'center' }}>
              <Typography level="body-xs">Future Value</Typography>
              <Typography level="h4" color="success">
                £
                {savingsResult.toLocaleString(undefined, {
        ...
        >
          <FormControl>
            <FormLabel sx={{ mb: 0.5 }}>Loan Amount (£)</FormLabel>
            <Input
              size="sm"
              type="number"
              value={loanPrincipal}
          ...
          <Button size="sm" color="primary" onClick={calculateLoan}>
            Calculate
          </Button>
          {loanResult !== null && (
            <Box sx={{ p: 1, bgcolor: 'primary.softBg', borderRadius: 'md', textAlign: 'center' }}>
              <Typography level="body-xs">Monthly Payment</Typography>
              <Typography level="h4" color="primary">
                £
                {loanResult.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </Typography>
        ...
        >
          <FormControl>
            <FormLabel sx={{ mb: 0.5 }}>Mortgage Amount (£)</FormLabel>
            <Input
              size="sm"
              type="number"
          ...
          <FormControl>
            <FormLabel sx={{ mb: 0.5 }}>Repayment Type</FormLabel>
            <RadioGroup
              row
              value={mortgageType}
              ...
              sx={{ gap: 2 }}
            >
              <Radio size="sm" value="repayment" label="Repayment" variant="outlined" />
              <Radio size="sm" value="interestOnly" label="Interest Only" variant="outlined" />
            </RadioGroup>
          </FormControl>
          <Button size="sm" color="danger" onClick={calculateMortgage}>
            Calculate
          </Button>
          {mortgageResult && (
            <Box
            ...
            >
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography level="body-xs">Interest:</Typography>
                <Typography level="body-xs" fontWeight="bold">
                  £{mortgageResult.totalInterest.toLocaleString()}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography level="body-xs">Total:</Typography>
                <Typography level="body-xs" fontWeight="bold">
                  £{mortgageResult.totalPaid.toLocaleString()}
                </Typography>
              </Box>
            </Box>
          )}
