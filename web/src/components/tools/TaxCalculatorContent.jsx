  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'auto' }}>
      ...
      >
        <TabList
          disableUnderline
          sx={{ p: 0.5, gap: 0.5, bgcolor: 'background.level1', mx: 1, mt: 1 }}
        >
          <Tab
            disableIndicator
            variant={subTab === 0 ? 'solid' : 'plain'}
            color="warning"
            value={0}
            sx={{ flex: 1, minHeight: 32, px: 1 }}
          >
            <Payments sx={{ mr: 0.5, fontSize: 18 }} /> Income Tax
          </Tab>
          <Tab
            disableIndicator
            variant={subTab === 1 ? 'solid' : 'plain'}
            color="warning"
            value={1}
            sx={{ flex: 1, minHeight: 32, px: 1 }}
          >
            <ReceiptLong sx={{ mr: 0.5, fontSize: 18 }} /> Stamp Duty
          </Tab>
        </TabList>
        ...
        <TabPanel value={0} sx={{ p: 1.5, flexGrow: 1, overflow: 'auto' }}>
          <Stack gap={1}>
            ...
            <Box sx={{ display: 'flex', gap: 1.5 }}>
              <FormControl sx={{ flex: 1 }}>
                <FormLabel sx={{ mb: 0.25, fontSize: 'xs' }}>Salary (£)</FormLabel>
                <Input
                  size="sm"
                  type="number"
                  value={salary}
              ...
              <FormControl sx={{ flex: 1 }}>
                <FormLabel sx={{ mb: 0.25, fontSize: 'xs' }}>Pension %</FormLabel>
                <Input
                  size="sm"
                  type="number"
                  value={pension}
            ...
            <Box sx={{ display: 'flex', gap: 1.5 }}>
              <FormControl sx={{ flex: 1 }}>
                <FormLabel sx={{ mb: 0.25, fontSize: 'xs' }}>Deductions (£)</FormLabel>
                <Input
                  size="sm"
                  type="number"
                  value={allowances}
              ...
              <FormControl sx={{ flex: 1 }}>
                <FormLabel sx={{ mb: 0.25, fontSize: 'xs' }}>Tax Code</FormLabel>
                <Input
                  size="sm"
            ...
            <Button size="sm" color="warning" onClick={calculateTax}>
              Calculate Take Home
            </Button>
            ...
            {result && (
              ...
              >
                <Typography level="body-xs" sx={{ textAlign: 'center', mb: 0.5 }}>
                  MONTHLY TAKE HOME
                </Typography>
                <Typography level="h3" color="warning" sx={{ textAlign: 'center', mb: 1 }}>
                  £{result.monthly.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                </Typography>
                <Stack gap={0.25}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography level="body-xs">Income Tax:</Typography>
                    <Typography level="body-xs" fontWeight="bold">
                      £{result.incomeTax.toLocaleString()}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography level="body-xs">NI:</Typography>
                    <Typography level="body-xs" fontWeight="bold">
                      £{result.ni.toLocaleString()}
                    </Typography>
                  </Box>
                  {result.sl > 0 && (
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Typography level="body-xs">Student Loan:</Typography>
                      <Typography level="body-xs" fontWeight="bold">
                        £{result.sl.toLocaleString()}
                      </Typography>
                    </Box>
                  )}
                  <Divider sx={{ my: 0.25 }} />
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography level="body-xs">Annual Take Home:</Typography>
                    <Typography level="body-xs" fontWeight="bold">
        ...
        <TabPanel value={1} sx={{ p: 1.5, flexGrow: 1, overflow: 'auto' }}>
          <Stack gap={1.5}>
            <FormControl>
              <FormLabel sx={{ mb: 0.5, fontSize: 'xs' }}>Property Price (£)</FormLabel>
              <Input
                size="sm"
                type="number"
                value={propertyPrice}
            ...
            <FormControl>
              <FormLabel sx={{ mb: 0.5, fontSize: 'xs' }}>Buyer Context</FormLabel>
              <RadioGroup
                value={buyerType}
            ...
            <Button size="sm" color="warning" onClick={calculateSDLT}>
              Calculate Stamp Duty
            </Button>
            {sdltVal !== null && (
              <Box
                sx={{ p: 1, bgcolor: 'warning.softBg', borderRadius: 'md', textAlign: 'center' }}
              >
                <Typography level="body-xs">Stamp Duty Payable</Typography>
                <Typography level="h4" color="warning">
                  £{sdltVal.toLocaleString()}
                </Typography>
              </Box>
            )}
          </Stack>
