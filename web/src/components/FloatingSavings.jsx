...
import { 
  Close, DragIndicator, OpenInNew, Savings
} from '@mui/icons-material';
import SavingsContent from './tools/SavingsContent';

export default function FloatingSavings({ onClose, isPopout = false, isDark = false, isDocked = false, onPopout, api, householdId }) {
  ...
  return (
    ...
    >
      <Box onMouseDown={onMouseDown} sx={{ p: 1, bgcolor: 'background.level2', color: 'text.primary', display: 'flex', alignItems: 'center', cursor: isDocked ? 'default' : 'move' }}>
        ...
        <Savings fontSize="small" sx={{ mr: 1, color: 'success.plainColor' }} />
        <Typography level="title-sm" sx={{ flexGrow: 1, color: 'inherit' }}>Quick Savings</Typography>
      ...
      <Box sx={{ flexGrow: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <SavingsContent onClose={onClose} api={api} householdId={householdId} isDark={isDark} />
      </Box>
    </Sheet>
  );
}