    ...
    <ListItem sx={{ p: 0 }}>
      <ListItemButton
        ...
        sx={{
          ...
          '&.Mui-selected': {
            background: (theme) =>
              `linear-gradient(135deg, ${theme.vars.palette.primary[400]} 0%, ${theme.vars.palette.primary[600]} 100%)`,
            color: '#fff',
...
export default function NavSidebar({
  ...
  useEffect(() => {
    ...
    if (currentPanelCategory === 'finance') {
      api
        .get(`/households/${household.id}/finance/profiles`)
    ...
    if (currentPanelCategory === 'chores') {
      api
        .get(`/households/${household.id}/chores`)
        .then((res) => {
          setPendingChores(
            (res.data || []).filter((c) => {
  ...
  const isSettingsTabActive = (tab) =>
    location.pathname.includes('/settings') && searchParams.get('tab') === String(tab);
    ...
    >
      ...
      >
        <Box sx={{ width: '100%', flexShrink: 0 }}>
          ...
          <Box sx={{ mb: 2, display: 'flex', justifyContent: 'center' }}>
            <Tooltip title="Dashboard" variant="soft" placement="right">
              <Avatar
                ...
                size="lg"
                onClick={() => {
                  navigate(`/household/${household.id}/dashboard`);
                  setHoveredCategory(null);
                }}
          ...
          >
            <RailIcon
              ...
              category="household"
              hasSubItems
              to={`/household/${household.id}/house`}
              location={location}
        ...
        >
          ...
          >
            ...
            <RailIcon
              ...
              category="finance"
              hasSubItems
              to={`/household/${household.id}/finance`}
              location={location}
              activeCategory={activeCategory}
              hoveredCategory={hoveredCategory}
              onHover={setHoveredCategory}
            ...
            <RailIcon
              ...
              label="Groceries"
              category="shopping"
              to={`/household/${household.id}/shopping`}
              location={location}
              activeCategory={activeCategory}
              hoveredCategory={hoveredCategory}
              onHover={setHoveredCategory}
            ...
            <RailIcon
              ...
              label="Chores"
              category="chores"
              to={`/household/${household.id}/chores`}
              location={location}
              activeCategory={activeCategory}
              hoveredCategory={hoveredCategory}
              onHover={setHoveredCategory}
            ...
            {enabledModules.includes('meals') && (
              <RailIcon
                ...
                label="Meals"
                category="meals"
                to={`/household/${household.id}/meals`}
                location={location}
                activeCategory={activeCategory}
                hoveredCategory={hoveredCategory}
                onHover={setHoveredCategory}
        ...
        >
          ...
          <Tooltip
            title={isMobile ? 'Account' : 'Account & Settings'}
            variant="soft"
            placement="right"
          >
            <IconButton
              ...
              aria-label="Account"
              color={
                hoveredCategory === 'account' || activeCategory === 'account'
                  ? 'primary'
                  : 'neutral'
              }
              onClick={(e) => {
              ...
              sx={{
                ...
                borderRadius: '50%',
                bgcolor:
                  userMenuAnchor || hoveredCategory === 'account' || activeCategory === 'account'
                    ? 'primary.softBg'
                    : 'transparent',
              }}
            >
          ...
          <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 1 }}>
            {currentPanelCategory === 'dashboard' && (
              <List sx={{ gap: 1 }}>
                <SubItem
                  label="Overview"
                  to={`/household/${household.id}/dashboard`}
                  emoji="🏠"
                  onClick={handleSubItemClick}
                />
                <Divider sx={{ my: 1 }} />
                <Typography level="body-xs" sx={{ px: 2, mb: 1, opacity: 0.6 }}>
                  QUICK STATS
                </Typography>
                <Box sx={{ px: 2 }}>
            ...
            {currentPanelCategory === 'household' && (
              <>
                ...
                <SubItem
                  ...
                  to={`/household/${household.id}/house/details`}
                  emoji="📋"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="People & Residents"
                  to={`/household/${household.id}/people`}
                  emoji="👥"
                  onClick={handleSubItemClick}
                />
                <Divider sx={{ my: 1 }} />
                ...
                {members
                  ...
                  .map((m) => (
                    <SubItem
                      key={m.id}
                      label={m.alias || (m.name || '').split(' ')[0]}
                      to={`/household/${household.id}/people/${m.id}`}
                      emoji={m.emoji}
                      onClick={handleSubItemClick}
                    />
                  ))}
                    ...
                    {members
                      ...
                      .map((m) => (
                        <SubItem
                          key={m.id}
                          label={m.name}
                          to={`/household/${household.id}/pets/${m.id}`}
                          emoji={m.emoji}
                          onClick={handleSubItemClick}
                        />
                      ))}
                ...
                {vehicles.map((v) => (
                  <SubItem
                    ...
                    to={`/household/${household.id}/vehicles/${v.id}`}
                    emoji={v.emoji}
                    onClick={handleSubItemClick}
                  />
                ))}
              ...
              <>
                ...
                >
                  ...
                  <IconButton
                    size="sm"
                    variant="plain"
                    onClick={() => setProfileCreateOpen(true)}
                      ...
                      <ListItemDecorator>
                        <Avatar
                          ...
                          sx={{
                            '--Avatar-size': '32px',
                            fontSize: '1.2rem',
                            bgcolor: getEmojiColor(p.emoji, isDark),
                          }}
                        >
                          {p.emoji}
                        </Avatar>
                ...
                <SubItem
                  label="Income"
                  to={getFinanceLink('income')}
                  emoji="💰"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Banking"
                  to={getFinanceLink('banking')}
                  emoji="🏦"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Savings"
                  to={getFinanceLink('savings')}
                  emoji="🐷"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Investments"
                  to={getFinanceLink('invest')}
                  emoji="📈"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Pensions"
                  to={getFinanceLink('pensions')}
                  emoji="👴"
                  onClick={handleSubItemClick}
                />
                <Divider sx={{ my: 1 }} />
                ...
                <SubItem
                  label="Credit Cards"
                  to={getFinanceLink('credit')}
                  emoji="💳"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Loans"
                  to={getFinanceLink('loans')}
                  emoji="📝"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Car Finance"
                  to={getFinanceLink('car')}
                  emoji="🚗"
                  onClick={handleSubItemClick}
                />
                <SubItem
                  label="Mortgages"
                  to={getFinanceLink('mortgage')}
                  emoji="🏠"
                  onClick={handleSubItemClick}
                />
              </>
            ...
            {currentPanelCategory === 'shopping' && (
              <Box sx={{ p: 1 }}>
                <Typography level="body-xs" sx={{ px: 1, mb: 1, fontWeight: 'bold' }}>
                  QUICK ADD
                </Typography>
                <Input
            ...
            {currentPanelCategory === 'chores' && (
              <Box sx={{ p: 1 }}>
                <Button
                  fullWidth
                  startDecorator={<Add />}
                  onClick={() => navigate(`/household/${household.id}/chores`)}
                  sx={{ mb: 2 }}
                >
                  Add Task
                </Button>
                ...
                <List sx={{ gap: 0.5 }}>
                  {pendingChores.length === 0 && (
                    <Typography level="body-xs" sx={{ px: 2, opacity: 0.5 }}>
                      All chores complete!
                    </Typography>
                  )}
                  {pendingChores.map((c) => (
            ...
            {currentPanelCategory === 'meals' && (
              <Box sx={{ p: 1 }}>
                ...
                <SubItem
                  label="Recipe book"
                  to={`/household/${household.id}/meals?tab=library`}
                  emoji="📖"
                  onClick={handleSubItemClick}
                />
              </Box>
              ...
              <>
                <Box sx={{ p: 2 }}>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    >
                      ...
                      <Button value="dark" sx={{ flex: 1 }} startDecorator={<DarkMode />}>
                        Dark
                      </Button>
                      <Button
                        value="system"
                ...
                <List sx={{ p: 1, gap: 0.5 }}>
                  <SubItem
                    ...
                    to={`/household/${household.id}/settings?tab=0`}
                    emoji={<Person />}
                    active={isSettingsTabActive(0)}
                    onClick={handleSubItemClick}
                  />
                  <SubItem
                    label="Security & MFA"
                    to={`/household/${household.id}/settings?tab=1`}
                    emoji={<Security />}
                    active={isSettingsTabActive(1)}
                    onClick={handleSubItemClick}
                  />
                  <SubItem
                    label="Appearance"
                    to={`/household/${household.id}/settings?tab=3`}
                    emoji={<Palette />}
                    active={isSettingsTabActive(3)}
                    onClick={handleSubItemClick}
                  />
                  ...
                  <SubItem
                    label="User Settings"
                    to={`/household/${household.id}/settings`}
                    emoji={<SettingsIcon />}
                    onClick={handleSubItemClick}
                  />
                  <SubItem
                    label="Household Settings"
                    to={`/household/${household.id}/house/details?tab=modules`}
                    emoji={<HomeWork />}
                    onClick={handleSubItemClick}
                  />
                  ...
                  <Box sx={{ mt: 'auto', pt: 2 }}>
                    ...
                    <SubItem
                      label="Log Out"
                      onClick={() => confirmAction('Log Out', 'Are you sure?', onLogout)}
                      emoji={<LogoutIcon color="danger" />}
                    />
                  </Box>
                </List>
              </>
        ...
        <ModalDialog>
          ...
          <DialogContent>
            <form onSubmit={handleCreateProfile}>
              ...
              <DialogActions>
                <Button variant="plain" color="neutral" onClick={() => setProfileCreateOpen(false)}>
                  Cancel
                </Button>
