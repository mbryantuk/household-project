...
export default function PasskeyManager({ api, showNotification }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      console.error(err);
      showNotification(
        err.response?.data?.error || err.message || 'Passkey registration failed.',
        'danger'
      );
    } finally {
      setLoading(false);
  ...
  return (
    <Sheet variant="outlined" sx={{ p: 3, borderRadius: 'md', bgcolor: 'background.level1' }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Box>
          <Typography level="title-md" startDecorator={<Fingerprint color="primary" />}>
            Passkeys
          </Typography>
      ...
      <List sx={{ '--ListItem-paddingY': '12px' }}>
        {passkeys.map((pk, idx) => (
          <ListItem
            key={pk.id}
            sx={{
              borderBottom: idx < passkeys.length - 1 ? '1px solid' : 'none',
              borderColor: 'divider',
            }}
          >
            <ListItemDecorator>
