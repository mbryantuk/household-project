...
export default function ChargesView({ financialProfileId }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [res, objRes] = await Promise.all([
        api.get(
          `/households/${householdId}/finance/recurring-costs?financial_profile_id=${financialProfileId}`
        ),
  ...
  const handleDelete = (id) => {
    confirmAction(
      'Delete Charge?',
      'This will remove the recurring cost from your budget.',
      async () => {
        try {
  ...
  return (
    <Box>
      ...
      <Sheet variant="outlined" sx={{ borderRadius: 'md', overflow: 'auto' }}>
        <Table
          aria-label="recurring charges table"
              ...
              <tr key={charge.id}>
                ...
                <td>
                  <Chip size="sm" variant="soft">
                    {resolveEntityName(charge.object_type, charge.object_id)}
                  </Chip>
                </td>
                <td>
                  <Chip
            ...
            <form onSubmit={handleSave}>
              <Stack spacing={2}>
                <FormControl required>
                  ...
                  <Input name="name" defaultValue={charges.find((c) => c.id === editingId)?.name} />
                </FormControl>
                <Grid container spacing={2}>
                  <Grid xs={6}>
                    <FormControl required>
                      ...
                      <Input
                        ...
                        type="number"
                        step="0.01"
                        defaultValue={charges.find((c) => c.id === editingId)?.amount}
                      />
                    </FormControl>
                  </Grid>
                  <Grid xs={6}>
                    <AppSelect
                      label="Category"
                      name="category"
                      defaultValue={charges.find((c) => c.id === editingId)?.category || 'Bill'}
                      options={[
                  ...
                  <Grid xs={6}>
                    <AppSelect
                      label="Frequency"
                      name="frequency"
                      defaultValue={charges.find((c) => c.id === editingId)?.frequency || 'monthly'}
                      options={[
                  ...
                  <Grid xs={6}>
                    <FormControl required>
                      ...
                      <Input
                        ...
                        min="1"
                        max="31"
                        defaultValue={charges.find((c) => c.id === editingId)?.payment_day}
                      />
                    </FormControl>
                  </Grid>
                </Grid>
                <Checkbox
                  label="Adjust for Nearest Working Day"
                  name="adjust_for_working_day"
                  defaultChecked={
                    charges.find((c) => c.id === editingId)?.adjust_for_working_day !== 0
                  }
                  value="1"
                />
