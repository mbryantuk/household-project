...
export default function IncomeView({ financialProfileId }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [incRes, bankRes] = await Promise.all([
        api.get(
          `/households/${householdId}/finance/income?financial_profile_id=${financialProfileId}`
        ),
        api.get(
          `/households/${householdId}/finance/current-accounts?financial_profile_id=${financialProfileId}`
        ),
      ]);
  ...
  const handleDelete = useCallback(
    async (id) => {
      confirmAction(
        'Delete Income',
        'Delete this income source? This will remove it from your budget.',
        async () => {
          try {
                ...
                <Grid container spacing={1}>
                  <Grid xs={6}>
                    ...
                    <Typography level="body-sm" fontWeight="bold">
                      {nextPayDate
                        ? `${format(nextPayDate, 'do MMM')} (${getDaysUntil(nextPayDate)}d)`
                        : 'N/A'}
                    </Typography>
                  </Grid>
                  <Grid xs={6}>
                    ...
                    <Typography level="body-sm">
                      {a.gross_annual_salary ? formatCurrency(a.gross_annual_salary) : '-'}
                    </Typography>
                  </Grid>
                </Grid>
              </FinanceCard>
      ...
      <Modal open={Boolean(selectedIncomeId)} onClose={() => setIncomeId(null)}>
        <ModalDialog sx={{ maxWidth: 800, width: '100%', maxHeight: '95vh', overflowY: 'auto' }}>
          <Box sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'flex-start' }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              <DialogTitle>
                {selectedIncomeId === 'new'
                  ? 'Add Income Source'
                  : `Edit ${selectedIncome?.employer}`}
              </DialogTitle>
              <Typography level="body-sm" color="neutral">
                Capture salary details and payment schedules.
              </Typography>
            </Box>
          </Box>
          <DialogContent sx={{ overflowX: 'hidden' }}>
            <form onSubmit={handleSubmit}>
              <Grid container spacing={2} sx={{ mt: 1 }}>
                <Grid xs={12} md={6}>
                  <FormControl required>
                    ...
                    <Input name="employer" defaultValue={selectedIncome?.employer} />
                  </FormControl>
                </Grid>
                <Grid xs={12} md={6}>
                  <FormControl>
                    <FormLabel>Role / Job Title</FormLabel>
                    <Input name="role" defaultValue={selectedIncome?.role} />
                  </FormControl>
                </Grid>
                <Grid xs={12} md={6}>
                  <AppSelect
                    ...
                    defaultValue={selectedIncome?.employment_type || 'employed'}
                    options={[
                ...
                <Grid xs={12} md={6}>
                  <AppSelect
                    label="Work Type"
                    name="work_type"
                    defaultValue={selectedIncome?.work_type || 'full_time'}
                    options={[
                ...
                <Grid xs={12} md={6}>
                  <AppSelect
                    ...
                    name="member_id"
                    defaultValue={String(selectedIncome?.member_id || '')}
                    options={members
                      .filter((m) => m.type !== 'pet')
                      .map((m) => ({ value: String(m.id), label: m.alias || m.name }))}
                ...
                <Grid xs={12} md={6}>
                  <AppSelect
                    ...
                    defaultValue={String(selectedIncome?.bank_account_id || '')}
                ...
                <Grid xs={12} sm={6} md={3}>
                  <FormControl>
                    ...
                    <Input
                      ...
                      defaultValue={selectedIncome?.gross_annual_salary}
                    />
                  </FormControl>
                </Grid>
                <Grid xs={12} sm={6} md={3}>
                ...
                <Grid xs={12} sm={6} md={3}>
                  <AppSelect
                    label="Frequency"
                    name="frequency"
                    defaultValue={selectedIncome?.frequency || 'monthly'}
                    options={[
                ...
                <Grid xs={12} sm={6} md={3}>
                  <FormControl required>
                    ...
                    <Input
                      ...
                      min="1"
                      max="31"
                      defaultValue={selectedIncome?.payment_day}
                      placeholder="e.g. 25"
                    />
                  </FormControl>
                </Grid>
                <Grid xs={12}>
                  <FormControl>
                    ...
                    <Input
                      name="addons"
                      defaultValue={selectedIncome?.addons}
                ...
                <Grid xs={12}>
                  <FormControl>
                    ...
                    <Input name="notes" defaultValue={selectedIncome?.notes} />
                  </FormControl>
                </Grid>
                <Grid xs={12}>
                  <Stack direction="row" spacing={2}>
                    <Checkbox
                      ...
                      name="is_primary"
                      defaultChecked={selectedIncome?.is_primary === 1}
                      value="1"
                    />
                    <Checkbox
                      label="Nearest Working Day (Prior)"
                      name="nearest_working_day"
                      defaultChecked={selectedIncome?.nearest_working_day !== 0}
                      value="1"
                    />
                  </Stack>
                </Grid>
              ...
              <DialogActions sx={{ mt: 2 }}>
                ...
                <Button type="submit" variant="solid" sx={{ height: '44px' }}>
                  Save Income
                </Button>
              </DialogActions>
            </form>
