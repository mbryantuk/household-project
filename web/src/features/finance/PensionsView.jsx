...
export default function PensionsView({ financialProfileId }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [penRes, assRes] = await Promise.all([
        api.get(
          `/households/${householdId}/finance/pensions?financial_profile_id=${financialProfileId}`
        ),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Promise.all(
        toRemove.map((mid) =>
          api.delete(
            `/households/${householdId}/finance/assignments/finance_pensions/${itemId}/${mid}`
          )
        )
      );
  ...
  const handleDelete = async (id) => {
    confirmAction(
      'Delete Pension',
      'Are you sure you want to delete this pension? This cannot be undone.',
      async () => {
        try {
  ...
  const handleAssignMember = async (memberId) => {
    try {
      ...
      const assRes = await api.get(
        `/households/${householdId}/finance/assignments?entity_type=finance_pensions`
      );
      setAssignments(assRes.data || []);
    } catch (err) {
  ...
  const handleUnassignMember = async (memberId) => {
    try {
      await api.delete(
        `/households/${householdId}/finance/assignments/finance_pensions/${assignItem.id}/${memberId}`
      );
      const assRes = await api.get(
        `/households/${householdId}/finance/assignments?entity_type=finance_pensions`
      );
      ...
      <Modal open={Boolean(selectedPensionId)} onClose={() => setPensionId(null)}>
        <ModalDialog sx={{ width: '100%', maxWidth: 500, maxHeight: '95vh', overflowY: 'auto' }}>
          <Box sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'flex-start' }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              <DialogTitle>
                {selectedPensionId === 'new' ? 'Add Pension' : 'Edit Pension'}
              </DialogTitle>
            ...
            <form onSubmit={handleSubmit}>
              <Stack spacing={2} sx={{ mt: 1 }}>
                <FormControl required>
                  ...
                  <Input
                    name="plan_name"
                    defaultValue={selectedPension?.plan_name}
                ...
                <Grid container spacing={2}>
                  <Grid xs={6}>
                    <FormControl required>
                      ...
                      <Input
                        name="provider"
                        defaultValue={selectedPension?.provider}
                  ...
                  <Grid xs={6}>
                    <FormControl>
                      ...
                      <Input
                        name="type"
                        defaultValue={selectedPension?.type}
                        placeholder="e.g. SIPP, Workplace"
                      />
                    </FormControl>
                  </Grid>
                </Grid>
                  ...
                  <Grid xs={6}>
                    <FormControl required>
                      ...
                      <Input
                        ...
                        defaultValue={selectedPension?.current_value}
                      />
                    </FormControl>
                  </Grid>
                  <Grid xs={6}>
                    <FormControl>
                      ...
                      <Input
                        ...
                        type="number"
                        slotProps={{ input: { step: 'any' } }}
                        defaultValue={selectedPension?.monthly_contribution}
                      />
                    </FormControl>
                  </Grid>
                  <Grid xs={6}>
                    <FormControl>
                      ...
                      <Input
                        ...
                        min="1"
                        max="31"
                        defaultValue={selectedPension?.payment_day}
                        placeholder="e.g. 1"
                      />
                    </FormControl>
                  </Grid>
              ...
              <Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between' }}>
                {selectedPensionId !== 'new' && (
                  <Button
                    color="danger"
                    variant="soft"
                    onClick={() => {
                      handleDelete(selectedPension.id);
                    }}
                  >
                    Delete
                  </Button>
                ...
                <Box sx={{ display: 'flex', gap: 1, ml: 'auto' }}>
                  <Button variant="plain" color="neutral" onClick={() => setPensionId(null)}>
                    Cancel
                  </Button>
