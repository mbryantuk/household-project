...
export default function MortgagesView({ financialProfileId }) {
  ...
  useEffect(() => {
    if (selectedMortgage) {
      setSelectedEmoji(
        selectedMortgage.emoji || (selectedMortgage.mortgage_type === 'equity' ? '💰' : '🏠')
      );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [mRes, assRes, assetRes, detRes] = await Promise.all([
        api.get(
          `/households/${householdId}/finance/mortgages?financial_profile_id=${financialProfileId}`
        ),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Promise.all(
        toRemove.map((mid) =>
          api.delete(
            `/households/${householdId}/finance/assignments/finance_mortgages/${itemId}/${mid}`
          )
        )
      );
  ...
  const handleDelete = async (id) => {
    confirmAction(
      'Delete Item',
      'Are you sure you want to delete this mortgage/equity loan? This cannot be undone.',
      async () => {
        try {
  ...
  const handleAssignMember = async (memberId) => {
    try {
      ...
      const assRes = await api.get(
        `/households/${householdId}/finance/assignments?entity_type=finance_mortgages`
      );
      setAssignments(assRes.data || []);
    } catch (err) {
  ...
  const handleUnassignMember = async (memberId) => {
    try {
      await api.delete(
        `/households/${householdId}/finance/assignments/finance_mortgages/${assignItem.id}/${memberId}`
      );
      const assRes = await api.get(
        `/households/${householdId}/finance/assignments?entity_type=finance_mortgages`
      );
  ...
  const properties = [
    {
      id: 'primary',
      name: household?.name || 'Primary Residence',
      valuation: houseDetails?.current_valuation || 0,
      emoji: '🏠',
    },
    ...assets
      .filter((a) => a.category?.toLowerCase() === 'property')
      .map((a) => ({
        id: a.id,
        name: a.name,
        valuation: a.purchase_value || 0,
        emoji: a.emoji || '🏘️',
      })),
  ];

  const groupedMortgages = properties
    .map((prop) => {
      ...
      <Stack spacing={4}>
        {groupedMortgages.map((prop) => {
          const totalDebt = prop.mortgages.reduce((sum, m) => {
            ...
            return (
              sum + (parseFloat(m.remaining_balance) || 0) + (parseFloat(m.other_secured_debt) || 0)
            );
          }, 0);
            ...
            <Box key={prop.id}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    ...
                    <Typography level="body-xs">
                      Valuation: {formatCurrency(prop.valuation)}
                    </Typography>
                  </Box>
                </Box>
              ...
              <Grid container spacing={2}>
                {prop.mortgages.map((mort) => {
                  ...
                  const h2bData = isEquityType
                    ? calculateH2BProjections(
                  ...
                  return (
                    <Grid xs={12} lg={isEquityType ? 12 : 6} key={mort.id}>
                      <FinanceCard
                        ...
                        subtitle={
                          isEquityType
                            ? `Equity Loan (${sharePercent.toFixed(1)}%)`
                            : 'Mortgage Part'
                        }
                        emoji={mort.emoji || (isEquityType ? '💰' : '🏠')}
                        isDark={isDark}
                        balance={isEquityType ? currentRedemption : main}
                        balanceColor="danger"
                        subValue={
                          isEquityType ? 'Est. Redemption' : formatPercent(mort.interest_rate)
                        }
                      ...
                      >
                        <Grid container spacing={1}>
                          <Grid xs={isEquityType ? 2 : 4}>
                            ...
                            >
                              {formatCurrency(mort.monthly_payment)}
                            </Typography>
                          </Grid>
                          <Grid xs={isEquityType ? 2 : 4}>
                            <Typography level="body-xs" color="neutral">
                          ...
                          <Grid xs={isEquityType ? 2 : 4}>
                            ...
                            >
                              {isEquityType
                                ? mort.equity_loan_start_date || 'N/A'
                                : mort.term_years
                                  ? mort.term_months > 0
                                    ? `${mort.term_years}y ${mort.term_months}m`
                                    : `${mort.term_years}y`
                                  : 'N/A'}
                            </Typography>
                          </Grid>
                          ...
                          <Stack spacing={1} mt={2}>
                            ...
                            >
                              ...
                              <Typography level="body-xs" sx={{ color: 'text.primary' }}>
                                Based on {formatCurrency(prop.valuation)} valuation, the
                                government's share is <b>{formatCurrency(currentRedemption)}</b>.
                              </Typography>
                            </Sheet>
                            <Sheet
                              variant="soft"
                            ...
                            >
                              ...
                              <Typography level="body-xs" sx={{ color: 'text.primary' }}>
                                To date (approx): <b>{formatCurrency(h2bData?.totalPaidSoFar)}</b>
                              </Typography>
                              <Typography level="body-xs" color="neutral">
                                Projected 25y total: {formatCurrency(h2bData?.totalProjectedLife)}
                              </Typography>
                            </Sheet>
                          </Stack>
                        )}
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'flex-start' }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              ...
              <Typography level="body-sm" color="neutral">
                Capture professional statement data with full decimal precision.
              </Typography>
            </Box>
          </Box>
          ...
          <DialogContent>
            <form onSubmit={handleSubmit}>
              ...
              <Stack spacing={2}>
                <Grid container spacing={2}>
                  <Grid xs={12} md={6}>
                    <AppSelect
                      ...
                      name="asset_id"
                      defaultValue={String(selectedMortgage?.asset_id || 'primary')}
                      options={[
                  ...
                  <Grid xs={12} md={6}>
                    <FormControl required>
                      ...
                      <Input
                        name="lender"
                        defaultValue={selectedMortgage?.lender}
                  ...
                  <>
                      ...
                      <Grid xs={12} sm={6} md={4}>
                        <FormControl required>
                          ...
                          <Input
                            ...
                            defaultValue={selectedMortgage?.total_amount}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6} md={4}>
                        <FormControl required>
                          ...
                          <Input
                            ...
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.remaining_balance}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6} md={4}>
                        <FormControl required>
                          ...
                          <Input
                            ...
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.interest_rate}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6} md={4}>
                        <FormControl required>
                          ...
                          <Input
                            ...
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.monthly_payment}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6} md={4}>
                        <FormControl>
                          ...
                          <Input
                            name="fixed_rate_expiry"
                            type="date"
                            defaultValue={selectedMortgage?.fixed_rate_expiry}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={6}>
                        <FormControl required>
                          ...
                          <Input
                            ...
                            min="1"
                            max="31"
                            defaultValue={selectedMortgage?.payment_day}
                          />
                        </FormControl>
                      </Grid>
                      ...
                      <Grid xs={6}>
                        <FormControl>
                          ...
                          <Input
                            name="term_years"
                            type="number"
                            defaultValue={selectedMortgage?.term_years}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={6}>
                        <FormControl>
                          ...
                          <Input
                            name="term_months"
                            type="number"
                            defaultValue={selectedMortgage?.term_months || 0}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12}>
                        <AppSelect
                          ...
                          name="repayment_type"
                          defaultValue={selectedMortgage?.repayment_type || 'Repayment'}
                          options={[
                    ...
                    <Divider>
                      <Chip variant="soft" color="warning" size="sm">
                        FOLLOW-ON RATE (AFTER FIXED ENDS)
                      </Chip>
                    </Divider>
                    <Grid container spacing={2}>
                      <Grid xs={12} sm={6}>
                        <FormControl>
                          <FormLabel>Expected Follow-on Rate (%)</FormLabel>
                          <Input
                            name="follow_on_rate"
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.follow_on_rate}
                      ...
                      <Grid xs={12} sm={6}>
                        <FormControl>
                          <FormLabel>Follow-on Monthly Payment (£)</FormLabel>
                          <Input
                            name="follow_on_payment"
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.follow_on_payment}
                            placeholder="e.g. 1187.36"
                          />
                        </FormControl>
                      </Grid>
                    </Grid>
                      ...
                      <Grid xs={12} sm={6}>
                        <FormControl required>
                          ...
                          <Input
                            ...
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.original_purchase_price}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6}>
                        <FormControl required>
                          ...
                          <Input
                            name="equity_loan_amount"
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.equity_loan_amount}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6}>
                        <FormControl required>
                          ...
                          <Input
                            name="equity_loan_start_date"
                            type="date"
                            defaultValue={selectedMortgage?.equity_loan_start_date}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6}>
                        <FormControl required>
                          <FormLabel>Estimated Future RPI (%)</FormLabel>
                          <Input
                            name="equity_loan_cpi_rate"
                            type="number"
                            slotProps={{ input: { step: 'any' } }}
                            defaultValue={selectedMortgage?.equity_loan_cpi_rate || 4.0}
                          />
                        </FormControl>
                      </Grid>
                      <Grid xs={12} sm={6}>
                ...
                <FormControl>
                  <FormLabel>Assign Borrowers / Owners</FormLabel>
              ...
              <Box sx={{ mt: 3, display: 'flex', justifyContent: 'space-between' }}>
                {selectedMortgageId !== 'new' && (
                  <Button
                    color="danger"
                    variant="soft"
                    onClick={() => handleDelete(selectedMortgage.id)}
                  >
                    Delete
                  </Button>
                )}
                <Box sx={{ display: 'flex', gap: 1, ml: 'auto' }}>
                  <Button variant="plain" color="neutral" onClick={() => setMortgageId(null)}>
                    Cancel
                  </Button>
                  ...
                  return (
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Typography sx={{ color: 'text.primary' }}>{m.alias || m.name}</Typography>
                      </Box>
                      {isAssigned ? (
                        <Button
                          size="sm"
