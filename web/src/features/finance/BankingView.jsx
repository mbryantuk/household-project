...
export default function BankingView({ financialProfileId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedAccountId === 'new') {
        ...
        await api.put(
          `/households/${householdId}/finance/current-accounts/${selectedAccountId}`,
          data
        );
      ...
      await Promise.all([
        ...
        ...toRemove.map((mid) =>
          api.delete(
            `/households/${householdId}/finance/assignments/current_account/${itemId}/${mid}`
          )
        ),
      ]);
  ...
  const handleAssignMember = async (memberId) => {
    try {
      ...
      const assRes = await api.get(
        `/households/${householdId}/finance/assignments?entity_type=current_account`
      );
      setAssignments(assRes.data || []);
    } catch (err) {
  ...
  const handleUnassignMember = async (memberId) => {
    try {
      await api.delete(
        `/households/${householdId}/finance/assignments/current_account/${assignItem.id}/${memberId}`
      );
      const assRes = await api.get(
        `/households/${householdId}/finance/assignments?entity_type=current_account`
      );
        ...
        toImport.map((s) => {
          ...
          if (s.is_income) {
            ...
            if (s.action === 'update' && s.existing_id) {
              return api.put(
                `/households/${householdId}/finance/income/${s.existing_id}`,
                incomeData
              );
            }
            ...
            if (s.action === 'update' && s.existing_id) {
              return api.put(
                `/households/${householdId}/finance/recurring-costs/${s.existing_id}`,
                data
              );
            }
    ...
    <Box sx={{ overflowX: 'hidden' }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              <DialogTitle>
                {selectedAccountId === 'new'
                  ? 'Add Bank Account'
                  : `Edit ${selectedAccount?.bank_name}`}
              </DialogTitle>
                ...
                <Grid xs={6}>
                  <FormControl>
                    ...
                    <Input
                      ...
                      type="number"
                      slotProps={{ input: { step: 'any' } }}
                      defaultValue={selectedAccount?.overdraft_limit}
                    />
                  </FormControl>
                </Grid>
        ...
        <ModalDialog size="sm">
          ...
          <DialogContent>
            <Typography level="body-sm" sx={{ mb: 2 }}>
              Who has access to {assignItem?.bank_name}?
            </Typography>
            <Stack spacing={1}>
              {members
                .filter((m) => m.type !== 'pet')
      ...
      <Modal open={importModalOpen} onClose={() => setImportModalOpen(false)}>
        <ModalDialog sx={{ maxWidth: 1200, width: '98vw', maxHeight: '95vh' }}>
                      ...
                      <Checkbox
                        ...
                        onChange={(e) =>
                          setSelectedSuggestions(
                            e.target.checked ? suggestions.map((s) => s.normalized) : []
                          )
                        }
                      />
                    </th>
                  ...
                  {suggestions.map((s, idx) => {
                    ...
                    return (
                      <tr key={s.normalized}>
                        <td>
                          <Checkbox
                            checked={selectedSuggestions.includes(s.normalized)}
                            onChange={(e) =>
                              setSelectedSuggestions((prev) =>
                                e.target.checked
                                  ? [...prev, s.normalized]
                                  : prev.filter((id) => id !== s.normalized)
                              )
                            }
                          />
                        </td>
                        ...
                        <td>
                          <Input
                            ...
                            type="number"
                            value={s.amount}
                            onChange={(e) => updateS({ amount: e.target.value })}
                            startDecorator="£"
                          />
                        </td>
                        <td>
                          <Select
                            size="sm"
                            value={s.object_type}
                            onChange={(_, val) => updateS({ object_type: val, object_id: null })}
                          >
                        ...
                        <td>
                          {s.object_type !== 'household' && (
                            <Select
                              ...
                              value={s.object_id}
                              placeholder="Select..."
                              onChange={(_, val) => updateS({ object_id: val })}
                            >
                              {s.object_type === 'member' &&
                                members
                                  .filter((m) => m.type !== 'pet')
                                  .map((m) => (
                              ...
                              {s.object_type === 'vehicle' &&
                                importVehicles.map((v) => (
                                  <Option key={v.id} value={v.id}>
                                    {v.make} ({v.registration})
                                  </Option>
                                ))}
                            </Select>
                          )}
                        ...
                        <td>
                          {!s.is_income && (
                            <Select
                              size="sm"
                              value={s.category_id}
                              onChange={(_, val) => updateS({ category_id: val })}
                            >
                        ...
                        <td>
                          <Select
                            size="sm"
                            variant="soft"
                            color={s.action === 'update' ? 'success' : 'primary'}
                            value={s.action}
                            onChange={(_, val) => updateS({ action: val })}
                          >
          ...
          <DialogActions>
            <Button variant="plain" color="neutral" onClick={() => setImportModalOpen(false)}>
              Cancel
            </Button>
            <Button
              variant="solid"
