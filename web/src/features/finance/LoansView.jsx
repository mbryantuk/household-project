...
export default function LoansView({ financialProfileId }) {
  ...
  const fetchLoans = useCallback(async () => {
    ...
    try {
      const [lRes, assRes] = await Promise.all([
        api.get(
          `/households/${householdId}/finance/loans?financial_profile_id=${financialProfileId}`
        ),
  ...
  const handleSave = async () => {
    try {
      ...
      const url = isNew
        ? `/households/${householdId}/finance/loans`
        : `/households/${householdId}/finance/loans/${selectedLoanId}`;
      ...
      await Promise.all([
        ...
        ...toRemove.map((mid) =>
          api.delete(`/households/${householdId}/finance/assignments/loan/${itemId}/${mid}`)
        ),
      ]);
      ...
      showNotification('Error saving loan.', 'danger');
    }
  };
            ...
            <FinanceCard
              ...
              onDelete={() =>
                confirmAction('Delete?', 'Are you sure?', () =>
                  api
                    .delete(`/households/${householdId}/finance/loans/${loan.id}`)
                    .then(() => fetchLoans())
                )
              }
            >
              <Grid container spacing={1}>
                <Grid xs={6}>
                  ...
                  <Typography level="body-sm">
                    {formatCurrency(loan.total_amount, household?.currency)}
                  </Typography>
                </Grid>
                <Grid xs={6}>
                  <Typography level="body-xs" color="neutral">
                  ...
                  <Typography level="body-sm" fontWeight="bold">
                    {formatCurrency(loan.monthly_payment, household?.currency)}
                  </Typography>
                </Grid>
              </Grid>
            </FinanceCard>
      ...
      <Modal open={Boolean(selectedLoanId)} onClose={() => setLoanId(null)}>
        <ModalDialog sx={{ maxWidth: 600, width: '100%', maxHeight: '95vh', overflowY: 'auto' }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'flex-start' }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
              <Typography level="h4">
                {selectedLoanId === 'new' ? 'New Loan' : 'Edit Loan'}
              </Typography>
              <Typography level="body-sm" color="neutral">
                Track personal loans and debts.
              </Typography>
            </Box>
          </Box>
          <Divider />
              ...
              <Grid container spacing={2}>
                ...
                <Grid xs={12} sx={{ display: 'flex', alignItems: 'center' }}>
                  <Checkbox
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, nearest_working_day: e.target.checked ? 1 : 0 })
                    }
                  />
                </Grid>
                <Grid xs={12}>
                  <FormControl>
                    <FormLabel>Assign Borrowers</FormLabel>
