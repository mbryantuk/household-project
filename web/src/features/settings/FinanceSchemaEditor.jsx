...
export default function FinanceSchemaEditor({ metadataSchema, onChange }) {
  ...
  const updateField = (idx, key, val) => {
    ...
    if (
      key === 'label' &&
      (currentFields[idx].name.startsWith('field_') || !currentFields[idx].name)
    ) {
  ...
  const availableCategories = [...(STANDARD_CATEGORIES[selectedType] || []), ...customCategories];

  return (
    <Stack spacing={3}>
      <Typography level="body-sm">
        Define custom fields (e.g., Policy Numbers, Renewal Dates) and categories for recurring
        costs. These will appear when editing costs for the selected object type.
      </Typography>
        ...
        <Sheet variant="outlined" sx={{ p: 2, borderRadius: 'md' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography level="title-sm">
              Custom Fields for {availableCategories.find((c) => c.id === selectedCategory)?.label}
            </Typography>
            <Button size="sm" startDecorator={<Add />} onClick={addField}>
              Add Field
            </Button>
          </Box>
          ...
          <Stack spacing={2}>
            {activeFields.map((field, idx) => (
              <Box key={idx} sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                ...
                <IconButton size="sm" color="danger" onClick={() => removeField(idx)}>
                  <Delete />
                </IconButton>
              </Box>
            ))}
