...
import { Box, Typography, Button, Stack, Sheet, Switch } from '@mui/joy';
  ...
  return (
    <Stack spacing={4} sx={{ maxWidth: 800 }}>
      <Box>
        ...
        <Typography level="body-sm">Manage configuration and data for {household?.name}</Typography>
      </Box>

      {/* Modules Section */}
      <Sheet variant="outlined" sx={{ p: 3, borderRadius: 'md', bgcolor: 'background.level1' }}>
        <Typography level="title-md" sx={{ mb: 1 }} startDecorator={<ViewModule color="primary" />}>
          Feature Modules
        </Typography>
        <Typography level="body-xs" color="neutral" sx={{ mb: 3 }}>
          Disabling a module hides its associated data and prevents new entries. Existing data is
          preserved.
        </Typography>
        <Stack spacing={2}>
      ...
      <Sheet
        variant="outlined"
        sx={{ p: 3, borderRadius: 'md', bgcolor: 'warning.softBg', borderColor: 'warning.200' }}
      >
        <Typography level="title-md" sx={{ mb: 1, color: 'warning.700' }}>
          System Diagnostics
        </Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box>
            ...
            <Typography level="body-xs" color="neutral">
              Enable verbose logging for troubleshooting (Performance impact).
            </Typography>
          </Box>
          <Switch
