...
export default function ChoresView() {
  ...
  } = useChores(api, householdId);
  const { data: stats = [], refetch: refetchStats } = useChoreStats(api, householdId);
  ...
  const blocker = useUnsavedChanges(isDirty && isModalOpen);
    ...
    try {
      const payload = {
        ...
        assigned_member_id: formData.assigned_member_id
          ? parseInt(formData.assigned_member_id)
          : null,
      };
  ...
  return (
    <PullToRefresh
      onRefresh={async () => {
        await Promise.all([refetchChores(), refetchStats()]);
      }}
    >
      <Box data-testid="chores-view" sx={{ width: '100%', mx: 'auto', pb: 10 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          {stats.map((stat) => (
            <Grid key={stat.id} xs={12} sm={6} md={4} lg={3}>
              <Sheet
                variant="outlined"
                sx={{ p: 2, borderRadius: 'md', display: 'flex', alignItems: 'center', gap: 2 }}
              >
                <Avatar size="lg" sx={{ bgcolor: getEmojiColor(stat.emoji || '👤', isDark) }}>
                  {stat.emoji || '👤'}
                </Avatar>
                <Box sx={{ flexGrow: 1 }}>
                  <Typography level="title-sm">{stat.name}</Typography>
                  <Typography level="body-xs" color="neutral">
                    {stat.tasks_completed} Tasks Done
                  </Typography>
                </Box>
                <Box sx={{ textAlign: 'right' }}>
                  <Typography level="h4" color="success">
                    {formatCurrency(stat.total_earned)}
                  </Typography>
                  <Typography level="body-xs" color="neutral">
                    Earned
                  </Typography>
                    ...
                    <tr key={chore.id}>
                      ...
                      <td>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          ...
                          <Box>
                            ...
                            {chore.description && (
                              <Typography level="body-xs">{chore.description}</Typography>
                            )}
                          </Box>
                        </Box>
                      </td>
                      ...
                      <td>
                        <Typography
                          level="body-sm"
                          startDecorator={
                            <EventRepeat sx={{ fontSize: '1rem', color: 'neutral.400' }} />
                          }
                        >
                      ...
                      <td>
                        {chore.next_due_date ? (
                          <Chip
                            size="sm"
                            variant="soft"
                            color={getDueDateColor(chore.next_due_date)}
                          >
        ...
        <AppBottomSheet
          ...
          title={isEditMode ? 'Edit Chore' : 'Add New Chore'}
        >
          <form onSubmit={handleSubmit}>
            <Stack spacing={2}>
              <FormSection title="General Information" icon="📋" showDivider={false}>
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <IconButton
                    variant="outlined"
                    onClick={() => setEmojiPickerOpen(true)}
                    sx={{ width: 48, height: 48, fontSize: '1.5rem', flexShrink: 0 }}
                  >
                    {formData.emoji}
                  </IconButton>
              ...
              <FormSection title="Assignment & Frequency" icon="🔁">
                  ...
                  <Grid xs={6}>
                    <FormControl>
                      ...
                      <Select
                        ...
                        onChange={(_e, v) => setFormData({ ...formData, frequency: v })}
                      >
              ...
              <FormSection title="Value & Deadline" icon="📅">
                <Grid container spacing={2}>
                  <Grid xs={6}>
                    <FormControl>
