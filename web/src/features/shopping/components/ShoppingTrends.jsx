...
export default function ShoppingTrends({ api, householdId }) {
    ...
    const processed = Object.entries(groups)
      .map(([, instances]) => {
        ...
        const sorted = instances.sort(
          (a, b) => new Date(a.week_start || a.created_at) - new Date(b.week_start || b.created_at)
        );
  ...
  return (
    <Card variant="outlined" sx={{ mt: 3 }}>
      <Typography level="title-md" startDecorator={<TrendingUp />}>
        Price Trends (Recurring Items)
      </Typography>
      <Typography level="body-xs" color="neutral" sx={{ mb: 2 }}>
        Compare current vs previous prices for frequently bought items.
      </Typography>
              ...
              <tr key={idx}>
                ...
                <td style={{ textAlign: 'center' }}>
                  <Chip
                    ...
                    startDecorator={
                      t.change > 0 ? <TrendingUp /> : t.change < 0 ? <TrendingDown /> : <Remove />
                    }
                  >
