...
export default function ShoppingSchedules({ api, householdId, showNotification, confirmAction }) {
  ...
  const handleToggleComplete = async (schedule) => {
    ...
    const totalCost = schedule.items.reduce(
    ...
    try {
      await api.post(
        `/households/${householdId}/shopping-list/schedules/${schedule.id}/toggle-complete`,
        {
  ...
  const handleDelete = (id) => {
    confirmAction(
      'Delete Schedule',
      'Are you sure you want to delete this recurring shopping list?',
      async () => {
        try {
  ...
  const addItemToSchedule = () => {
    ...
    setFormData((prev) => ({
      ...
      items: [
        ...prev.items,
        { ...newItem, estimated_cost: parseFloat(newItem.estimated_cost) || 0 },
      ],
    }));
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...all,
      pending: all.total - all.completed,
      progress: all.total > 0 ? (all.completed / all.total) * 100 : 0,
    };
  }, [schedules]);
    ...
    <Box>
      ...
      >
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography level="title-md" startDecorator={<ShoppingCart />}>
            Monthly Cycles
          </Typography>
          <Typography level="h4" color="primary">
            {formatCurrency(totals.pending)} left
          </Typography>
        </Box>
        <LinearProgress
          determinate
            ...
            >
              <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                ...
                <Box sx={{ display: 'flex', gap: 0.5 }}>
                  <IconButton size="sm" variant="plain" onClick={() => handleEdit(s)}>
                    <Edit />
                  </IconButton>
                  <IconButton
                    size="sm"
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                <FormControl sx={{ flex: 1 }}>
                  ...
                  <Select
                    ...
                    onChange={(_, v) => setFormData({ ...formData, frequency: v })}
                  >
                ...
                {formData.frequency !== 'monthly' ? (
                  <FormControl sx={{ flex: 1 }}>
                    ...
                    <Select
                      ...
                      onChange={(_, v) => setFormData({ ...formData, day_of_week: v })}
                    >
              ...
              <Stack spacing={1}>
                <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  <Input
                    ...
                    sx={{ width: 70 }}
                  />
                  <IconButton size="sm" variant="solid" onClick={addItemToSchedule}>
                    <Add />
                  </IconButton>
                </Box>
                <Box
                    ...
                    >
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <IconButton
                          size="sm"
                          color="danger"
                          onClick={() => removeItemFromSchedule(idx)}
                        >
                          <Delete />
                        </IconButton>
                      </Box>
