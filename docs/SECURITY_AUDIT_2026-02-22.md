...
*   **Verification:**
    ...
    *   Cross-tenant data leakage is structurally impossible at the SQL level unless the `household_id` parameter itself is manipulated *before* the DB connection is established (which is protected by Auth Middleware).
...
*   **Remediation:** Requires `npm audit fix --force`. This will upgrade `vite` to v6 and `eslint` to v9, which are **breaking changes**.
    *   *Recommendation:* Schedule a dedicated task for "Frontend Tooling Upgrade".
