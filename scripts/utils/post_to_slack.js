...
async function run() {
    ...
    const swaggerPaths = Object.keys(swaggerRaw.paths).flatMap(p => Object.keys(swaggerRaw.paths[p]).map(m => `${m.toUpperCase()} ${p}`));
    ...
    const apiBlocks = [
        ...
        { type: "section", text: { type: "mrkdwn", text: `*Health Check:* <!date^${Math.floor(Date.now()/1000)}^{date_num} {time_secs}|Just now>\n\n${coverageText}` } }
    ];
    ...
    if (integrityText || stressResult) {
        const intBlocks = [
            ...
            { type: "section", text: { type: "mrkdwn", text: `*Last Verification:* <!date^${Math.floor(Date.now()/1000)}^{date_num} {time_secs}|Just now>\n\n${integrityText}${stressResult ? (cov.results[stressResult] === 'PASS' ? "✅ *Concurrency Stable*" : "❌ *Concurrency Unstable*") : ""}` } }
        ];
